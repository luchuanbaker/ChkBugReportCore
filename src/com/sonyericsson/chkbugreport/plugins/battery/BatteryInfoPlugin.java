...
public class BatteryInfoPlugin extends Plugin {
    ...
    private void genStats(BugReportModule br, Chapter ch, Node node, boolean detectBugs, String csvPrefix) {
        ...
        Chapter wakeLock = new Chapter(br.getContext(), "Partial Wake Locks");
        ...
        // Prepare the Wakeup reason table
        Chapter wakeReason = new Chapter(br.getContext(), "Wake Reasons");
        // Wakeup reason Abort:Last active Wakeup Source: ipc000000f0_sensors.qcom: 4h 10m 32s 598ms (43681 times) realtime
        Pattern pWR = Pattern.compile("Wakeup reason (.*:?.*?): (.*?) \\((.*?) times\\) (.*?)");
        Table tgWR = new Table(Table.FLAG_SORT, wakeReason);
        tgWR.setCSVOutput(br, "battery_" + csvPrefix + "_wakereasons");
        tgWR.setTableName(br, "battery_" + csvPrefix + "_wakereasons");
        tgWR.addColumn("Wake Reason", null, Table.FLAG_ALIGN_RIGHT, "uid varchar");
        tgWR.addColumn("Count", null, Table.FLAG_ALIGN_RIGHT, "count int");
        tgWR.addColumn("Time", null, Table.FLAG_ALIGN_RIGHT, "time varchar");
        tgWR.addColumn("Time(ms)", null, Table.FLAG_ALIGN_RIGHT, "time_ms int");
        tgWR.begin();
        ...
        for (Node item : node) {
            ...
            } else if (line.startsWith("Wake lock")) {
                if (!line.contains("(nothing executed)")) {
                    Matcher m = pWL.matcher(line);
                    if (m.find()) {
                        String uidName = m.group(1);
            ...
            } else if (line.startsWith("Wakeup reason")) {
                if (!line.contains("(nothing executed)")) {
                    Matcher m = pWR.matcher(line);
                    if (m.find()) {
                        String name = m.group(1);
                        String sTime = m.group(2);
                        String sCount = m.group(3);
                        long ts = Util.parseRelativeTimestamp(sTime.replace(" ", ""));
                        tgWR.setNextRowStyle(colorizeTime(ts));
                        tgWR.addData(name);
                        tgWR.addData(sCount);
                        tgWR.addData(sTime);
                        tgWR.addData(new ShadedValue(ts));
                    } else {
                        System.err.println("Could not parse line: " + line);
                    }
                }
